...
ElementL Liste::iemeElement (unsigned int indice) const {
  ...
  for (unsigned int j = 0; j < indice; j++) {
      temp = temp->suivant;
      if (temp == nullptr) {cout << "Erreur Liste::iemeElement : indice trop grand"; exit(1);}
  }
  return temp->info;
}
...
void Liste::supprimerTete () {
  Cellule * temp = prem;
  prem = prem->suivant;
  if(prem != nullptr) prem->precedent = nullptr;
  else last = nullptr;
  delete temp;
}
...
void Liste::trier() {
    // Tri par insertion
    if (prem == nullptr || prem->suivant == nullptr) return ; // rien a faire si moins de 2 elements

    Cellule * dernieretriee = prem;
    Cellule * aplacer = prem->suivant;
    Cellule * c;

    while (aplacer != nullptr) { // boucle principale sur les elements a placer
        c = prem;
        ...
        if (c != aplacer) {  // cas 1
            if (precC != nullptr) precC->suivant = aplacer;
            else prem = aplacer;
            if (suivAPlacer == nullptr) last = precAPlacer;
